#!/bin/bash
# Start a fresh plikd instance configured with Path="/sub" for subpath e2e testing.
...
TMPDIR=$(mktemp -d /tmp/plik-e2e-subpath.XXXXXX)
echo "$TMPDIR" > /tmp/plik-e2e-subpath-tmpdir  # so global-teardown.js can find it

WEBAPP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
SERVER_BIN="${PLIKD_BIN:-$WEBAPP_DIR/../server/plikd}"
PORT="${PLIK_SUBPATH_PORT:-8586}"
...
# Write minimal config for subpath e2e testing
